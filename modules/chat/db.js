...
async function fetchConnectionSubscriptions(connection) {
    ...
    const results = await ddb.query({
        ...
        KeyConditionExpression: `${db.Connection.Channels.Key
            } = :connectionId and begins_with(${db.Connection.Channels.Range
            }, :channelEntity)`,
...
async function fetchChannelSubscriptions(channel) {
    ...
    const results = await ddb.query({
        TableName: db.Table,
        KeyConditionExpression: `${db.Channel.Connections.Key
            } = :channelId and begins_with(${db.Channel.Connections.Range
            }, :connectionEntity)`,
        ExpressionAttributeValues: {
